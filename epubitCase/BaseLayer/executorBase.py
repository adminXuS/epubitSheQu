...
class ExecutorBase:
    ...
    # 操作链
    def get_action_chains(self, operate, elementName, word= None):
        if  operate.lower() in "click":
            ActionChains(self.get_executor()).click(elementName).perform()
        elif operate.lower() in "move":
            ActionChains(self.get_executor()).move_to_element(elementName).perform()
        elif operate.lower() in "send":
            ActionChains(self.get_executor()).click(elementName).send_keys(word).perform()